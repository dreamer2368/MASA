...
MASA::ad_cns_2d_sutherlands<Scalar>::ad_cns_2d_sutherlands()
{
  this->mmsname = "ad_cns_2d_sutherlands";
  this->dimension = 2;
  ...
  this->register_var("Amu",&Amu);
  this->register_var("Bmu",&Bmu);
  this->register_var("Cmu",&Cmu);
  this->register_var("bulkViscMult",&bulkViscMult);
...
template <typename Scalar>
int MASA::ad_cns_2d_sutherlands<Scalar>::init_var()
{
  int err = 0;
  ...
  err += this->set_var("Amu",.918);
  err += this->set_var("Bmu",.918);
  err += this->set_var("Cmu",.918);
  err += this->set_var("bulkViscMult",.918);
  err += this->set_var("Pr",.918);
...
template <typename Scalar>
Scalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_q_rho_u(Scalar x1, Scalar y1)
{
  using std::pow;
  ...
  U[0] = eval_u(x, y);
  U[1] = eval_v(x, y);
  ADScalar RHO = eval_rho(x, y);
  ADScalar P = eval_p(x, y);

  // Temperature
  ADScalar T = P / RHO / R;

  // Viscosity
  ADScalar mu = Amu * pow(T, Bmu) / (T + Cmu);
  ...
  NumberVector<NDIM, NumberVector<NDIM, ADScalar> > Tau = mu * (GradU + transpose(GradU) + (bulkViscMult - 2./3.)*divergence(U)*Identity);
...
template <typename Scalar>
Scalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_q_rho_v(Scalar x1, Scalar y1)
{
  using std::pow;
...
// public, static method
template <typename Scalar>
Scalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_q_rho_e(Scalar x1, Scalar y1)
{
  using std::pow;
  ...
  Scalar CpDivPr = Gamma * R / (Pr * (Gamma - 1.));
  ADScalar k = CpDivPr * mu;
...
// public, static method
template <typename Scalar>
Scalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_q_rho(Scalar x1, Scalar y1)
{
  typedef DualNumber<Scalar, NumberVector<NDIM, Scalar> > FirstDerivType;
  typedef DualNumber<FirstDerivType, NumberVector<NDIM, FirstDerivType> > SecondDerivType;
  typedef SecondDerivType ADScalar;
...
inputScalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_u(inputScalar x, inputScalar y)
{
  using std::cos;
...
// public method
template <typename Scalar> template<typename inputScalar>
inputScalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_v(inputScalar x, inputScalar y)
{
  using std::cos;
...
// public method
template <typename Scalar> template<typename inputScalar>
inputScalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_p(inputScalar x, inputScalar y)
{
  using std::cos;

  inputScalar P = p_0 + p_x * cos(a_px * PI * x / L) * p_y * cos(a_py * PI * y / L);
  return P;
}

// public method
template <typename Scalar> template<typename inputScalar>
inputScalar MASA::ad_cns_2d_sutherlands<Scalar>::eval_rho(inputScalar x, inputScalar y)
{
  using std::cos;

  inputScalar RHO = rho_0 + rho_x * cos(a_rhox * PI * x / L) * rho_y * cos(a_rhoy * PI * y / L);
  return RHO;
}
...
MASA_INSTANTIATE_ALL(MASA::ad_cns_2d_sutherlands);
