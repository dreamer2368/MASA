...
template <typename Scalar>
Scalar MASA::periodic_ternary_2d<Scalar>::eval_q_state(Scalar x1,Scalar y1,int eq)
{
  using std::cos;
  using std::sin;
  ...
  Scalar source_rhoYI = raw_value(divergence(mI * nI * (U + V_I)));
  Scalar source_rhoYE = raw_value(divergence(mE * nE * (U + V_E)));
...
template <typename Scalar>
Scalar MASA::periodic_ternary_2d<Scalar>::eval_exact_state(Scalar x,Scalar y,int eq)
{
  using std::cos;
  using std::sin;
